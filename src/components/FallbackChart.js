...
const FallbackChart = ({ currentPrice, chartType = 'line' }) => {
  ...
  useEffect(() => {
    const generatePriceData = () => {
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        const change = (Math.random() - 0.5) * 300; // Random price change
        price += change;
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="w-full h-32 relative">
              {/* Chart Background */}
              <div className="absolute inset-0 bg-gray-900/50 rounded-lg border border-gray-600"></div>
              
              {/* Price Bars */}
              <div className="absolute inset-2 flex items-end justify-between gap-1">
                {priceData.map((point, index) => {
                  const height = Math.max(10, (point.price / Math.max(...priceData.map(p => p.price))) * 100);
